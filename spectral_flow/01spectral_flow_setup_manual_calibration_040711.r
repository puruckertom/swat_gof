...
pdf(file=pdfs.filename,width=10.5, height=8.5, bg="white")
  ...
  modeled.eff <- 1 - sum((obsflow.ts - modeledflow.ts)^2) / sum((obsflow.ts-mean(obsflow.ts))^2)
...
gof.mat.6 <-matrix(data = NA, nrow = 6, ncol = 1)
inv.err.var1 <- (sum((modeledflow.ts-obsflow.ts)^2)/Nsim)^(-1)
inv.err.var2 <- (sum((modeledflow.ts-obsflow.ts)^2)/Nsim)^(-2)
inv.err.var3 <- (sum((modeledflow.ts-obsflow.ts)^2)/Nsim)^(-3)
gof.mat.6[1,1] <- paste("iev1=",inv.err.var1)
gof.mat.6[2,1] <- paste("iev2=",inv.err.var2)
gof.mat.6[3,1] <- paste("iev3=",inv.err.var3)
nash.sut.1 <- (1 - sum((modeledflow.ts-obsflow.ts)^2) / sum((obsflow.ts-mean(obsflow.ts))^2))^1
nash.sut.2 <- (1 - sum((modeledflow.ts-obsflow.ts)^2) / sum((obsflow.ts-mean(obsflow.ts))^2))^2
nash.sut.3 <- (1 - sum((modeledflow.ts-obsflow.ts)^2) / sum((obsflow.ts-mean(obsflow.ts))^2))^3
gof.mat.6[4,1] <- paste("ns1=",nash.sut.1)
gof.mat.6[5,1] <- paste("ns2=",nash.sut.2)
gof.mat.6[6,1] <- paste("ns3=",nash.sut.3)
gof.mat <- rbind(gof.mat,gof.mat.6)

# write gof to file
write.table(gof.mat,file=paste(sf_dir,"sim_gofs.csv",sep=""),row.names=FALSE,sep=",")

title.text <- paste(somethingdescriptive," (",access.root,")",sep="")

# power spectrum - fast fourier transforms
Nsamps <- length(modeledflow.ts)
k = (2*pi*(1:((Nsamps/2) - 1)))/Nsamps
obs.Pk = abs(fft(obsflow.ts[1:(Nsamps/2) - 1], inverse = TRUE))^2
sim.Pk = abs(fft(modeledflow.ts[1:(Nsamps/2) - 1], inverse = TRUE))^2
