...
ID <- as.data.frame(cbind(ExtractID,xy,NcIDx,NcIDy)) #all conus pts (this is input for all ws)
ID.SPDF <- SpatialPointsDataFrame(ConusPts,ID) #all conus pts (this is input for all ws)

##To id pixels within watershed
WSCentroids.MC <- over(MC,ConusPts,returnList=T) #'over' doesn't work with SPDF - just use SP
WSCentroids.LR <- over(LR,ConusPts,returnList=T)
WSCentroids.NC <- over(NC,ConusPts,returnList=T)
WSCentroids.FB <- over(FB,ConusPts,returnList=T)
WSCentroids.MC <- as.data.frame(WSCentroids.MC)
WSCentroids.LR <- as.data.frame(WSCentroids.LR)
WSCentroids.NC <- as.data.frame(WSCentroids.NC)
WSCentroids.FB <- as.data.frame(WSCentroids.FB)

##write csvs of centroid coordinates and original NetCDF IDs
WSIDs.MC <- over(MC, ID.SPDF, returnList=T)
WSIDs.MC <- as.data.frame(WSIDs.MC)
WSIDs.MC <- WSIDs.MC[,-1]
write.csv(WSIDs.MC,file=paste(output_dir,"CoordInfo_MC.csv",sep=""))
WSIDs.LR <- over(LR, ID.SPDF, returnList=T)
WSIDs.LR <- as.data.frame(WSIDs.LR)
WSIDs.LR <- WSIDs.LR[,-1]
write.csv(WSIDs.LR,file=paste(output_dir,"CoordInfo_LR.csv",sep=""))
WSIDs.NC <- over(NC, ID.SPDF, returnList=T)
WSIDs.NC <- as.data.frame(WSIDs.NC)
WSIDs.NC <- WSIDs.NC[,-1]
write.csv(WSIDs.NC,file=paste(output_dir,"CoordInfo_NC.csv",sep=""))
WSIDs.FB <- over(FB, ID.SPDF, returnList=T)
WSIDs.FB <- as.data.frame(WSIDs.FB)
WSIDs.FB <- WSIDs.FB[,-1]
write.csv(WSIDs.FB,file=paste(output_dir,"CoordInfo_FB.csv",sep=""))

##What is the difference bt clipping processes above and below?


##write buffered points to shapefile
#dim(WSIDs.DF)
WS.MC <- ConusPts[MC,]
WS.MC.SPDF <- SpatialPointsDataFrame(WS.MC,WSIDs.MC)
writeOGR(WS.MC.SPDF, paste(output_dir, "Shapefiles", sep=""), "BufferedPtsMC", driver="ESRI Shapefile")
WS.LR <- ConusPts[LR,]
WS.LR.SPDF <- SpatialPointsDataFrame(WS.LR,WSIDs.LR)
writeOGR(WS.LR.SPDF, paste(output_dir, "Shapefiles", sep=""), "BufferedPtsLR", driver="ESRI Shapefile")
WS.NC <- ConusPts[NC,]
WS.NC.SPDF <- SpatialPointsDataFrame(WS.NC,WSIDs.NC)
writeOGR(WS.NC.SPDF, paste(output_dir, "Shapefiles", sep=""), "BufferedPtsNC", driver="ESRI Shapefile")
WS.FB <- ConusPts[FB,]
WS.FB.SPDF <- SpatialPointsDataFrame(WS.FB,WSIDs.FB)
writeOGR(WS.FB.SPDF, paste(output_dir, "Shapefiles", sep=""), "BufferedPtsFB", driver="ESRI Shapefile")

###To write *all* CONUS points to shapefile:
ConusSPDF <- SpatialPointsDataFrame(ConusPts,ID) 
#summary(ConusSPDF)
writeOGR(ConusSPDF, paste(root_dir,"Shapefiles",sep=""), "ConusPtsR", driver="ESRI Shapefile")

################################
##3. Create SPDFs with x#, y#, Lat, Long, columns for each timestep
################################
##Iteration 1: Run these for MC, then skip to startdate
Pixels <- WSCentroids.MC[,1]
...
##Iteration 2: Run these for LR, then skip to startdate
Pixels <- WSCentroids.LR[,1]
Npixels <- length(Pixels) #This is the number of pixels to extract data from, set length of SPDF to this
Pixels <- as.vector(Pixels)
## Skip

##Iteration 3: Run these for MC, then skip to startdate
Pixels <- WSCentroids.NC[,1]
Npixels <- length(Pixels) #This is the number of pixels to extract data from, set length of SPDF to this
Pixels <- as.vector(Pixels)
## Skip

##Iteration 4: Run these for FB, then skip to startdate
Pixels <- WSCentroids.FB[,1]
Npixels <- length(Pixels) #This is the number of pixels to extract data from, set length of SPDF to this
Pixels <- as.vector(Pixels)
## Skip
...
##Huzzah!
write.csv(WSPrecip, file=paste(output_dir,"MC2002.csv",sep=""))
