...
#prob=1/(1:5) - 1:5 is a shortcut for 1-5, shortcut for looping through prob 1/i i=1-5

## Check that sample probabilities look right by simulating 1000 random combinations
barplot( table( sample( letters[1:5], 1000, prob=1/(1:5), replace=TRUE ) ) )

## Randomly re-order the alphabet
sample( letters, 26, replace=FALSE) #replace=FALSE means values can't be reused, good for randomizing
sample( letters, 26, replace=TRUE) # replace=TRUE allows values to be reused
