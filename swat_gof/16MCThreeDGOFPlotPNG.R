...
threedplot.angle = 55
threedscale.z = 2
threed.mar <- c(1, 1,1, 1) + 0.1
png.filename <- paste(source_dir,"mcCL_MPE/3dpass_evolution.png",sep="")
png(file=png.filename,width=4, height=9,bg="white")
  ...
  sp3d.1 <- scatterplot3d(plot.this.thinned.1, type="h",highlight.3d=FALSE, angle=threedplot.angle, scale.z=4, 
                          pch=16, main="Pass 1    ",xlab="",ylab="",mar= c(1, 1,1, 1) + 0.1,col.axis="gray31",col.grid="lightblue",
...
png.filename <- paste(source_dir,"mcCL_MPE/3dpass_evolution_greyscale.png",sep="")
png(file=png.filename,width=3, height=9,bg="white")
par(mfrow=c(5,1),mgp=c(1,0,0))
