...
#library(spdep) #for spatial dependence - not sure 
library(proj4) #for reprojection
#library(raster)
library(geoR)
...
#MPE.root <- paste(root_dir,"ShapefileTest/",sep="")
...
#llNCEP_Sphere <- CRS("+proj=stere +lat_0=0 +lon_0=0 +k=1 +x_0=0 +y_0=0 +a=6371007 +b=6371007 +units=m +no_defs") # from somewhere (Tanya?) I originally had 6371000, other documentation (msprcpps) says r=6371007, Arc says 6371200
#Test1 <- readShapePoints(paste(root_dir,"ShapefileTest/ST4_2009010112_24h",sep=""),proj4string=llNCEP_Sphere)
#Test2 <- readShapePoints(paste(root_dir,"ShapefileTest/ST4_2009010112_24h",sep=""),proj4string=llHRAP)
#Test3 <- readShapePoints(paste(root_dir,"ShapefileTest/ST4_2009010112_24h",sep=""),proj4string=llStereo)
#Test4 <- readShapePoints(paste(root_dir,"ShapefileTest/ST4_2009010112_24h",sep=""),proj4string=llCRS)
#???projection specification doesn't matter, it's plotting by the lat-long info columns in the shapefile wo adjustment - is it assuming CRS?
...
#spplot(TheDairyState, zcol="POP2000")

#CRS is unprojected lat long, need proj3cted objects for geometric operations
###############
#Can I project SB1.poly to HRAP? (Apparently Not)
###############
#SB1HRAP <- project(SB1.poly, llHRAP) 
#nope

#SB1HRAP <- project(SB1.poly, "+proj=stere +lat_0=39.224079 +lon_0=-98.54180699999999 +k=1 +x_0=0 +y_0=0 +ellps=sphere +units=m +no_defs")
#nope

#SB1HRAP <- ptransform(SB1.poly, "+proj=longlat +ellps=WGS84 +datumWGS84 +no_defs", "+proj=stere +lat_0=39.224079 +lon_0=-98.54180699999999 +k=1 +x_0=0 +y_0=0 +ellps=sphere +units=m +no_defs", silent=F)
#nope, says input must be 2D with at most three columns

#####################
#So bring in an already projected shapefile to clip
#####################
USGS <- CRS("+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs")
SB1.USGS <- readShapePoly(paste(root_dir,"ShapefileTest/SB1_USGS",sep=""),proj4string=USGS)

#HRAP centroids
Centroids <- readShapePoints(paste(root_dir,"ShapefileTest/ST4_2009010112_24h",sep=""),proj4string=llHRAP)
DBFdata <- read.dbf(file=paste(root_dir,"ShapefileTest/ST4_2009010112_24h.dbf",sep="")) #to get Point IDs of corners

#####################
#Clip centroids to SB1 boundary
#####################
#http://gis.stackexchange.com/questions/46954/clip-spatial-object-to-bounding-box-in-r
#extent <- extent(Test1)
#trytoclip <- crop(Test1,extent)
#These don't work because they're intended for rasters, I assume

###Options from rgeos
ContList <- gContains(SB1.USGS, Centroids, byid=T) 
#takes a long time but works to ID which centroids are within sb1 (no clipping)

RelMatrix <- gRelate(SB1.USGS, Centroids, pattern=NULL, byid=F) #to build intersection matrix
#seems like overkill, haven't tried

###Options from geoR
polygrid #requires polygon as matrix
#possibly more options in geoR

###Options from sp
over(Centroids, SB1.USGS, returnList=T) #must be same projection, but this looks like it might be the winner

###############
#Can I project SB1.USGS to HRAP? (Same errors as unprojected SB1.poly)
###############
SB1HRAP <- project(SB1.USGS, llHRAP) 
#nope

SB1HRAP <- project(SB1.USGS, "+proj=stere +lat_0=39.224079 +lon_0=-98.54180699999999 +k=1 +x_0=0 +y_0=0 +ellps=sphere +units=m +no_defs")
#nope, "input must be 2-dimensional"

SB1HRAP <- ptransform(SB1.USGS, "+proj=longlat +ellps=WGS84 +datumWGS84 +no_defs", "+proj=stere +lat_0=39.224079 +lon_0=-98.54180699999999 +k=1 +x_0=0 +y_0=0 +ellps=sphere +units=m +no_defs", silent=F)
#nope, says input must be 2D with at most three columns
...
spplot(SB1.poly, col="darkseagreen4")
plot(Test1, col="green3", pch=5)
plot(Test2, add=TRUE, col="deepskyblue4", pch=8)
plot(Test3, add=TRUE, col="darkgoldenrod2", pch=20)
...
#############
#Try to read in as dbfs
#############

#TestDBF <- read.dbf(file=paste(root_dir,"ShapefileTest/ST4_2009010112_24h.dbf",sep="")) 
#TestDBF[1,]
#this works 
#these radar dbfs have column LAT and LON (presumably because of degrib output specs, not usu. in the dbf)
...
################################
SBBox1 <- bbox(SB1.poly)
S1 <- Spatial(bbox = SBBox1, proj4string = llCRS)