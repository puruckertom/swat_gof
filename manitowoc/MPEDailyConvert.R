...
if(.Platform$OS.type=="unix"){
  root_dir <- path.expand("~/Dropbox/...")
  output_dir <- path.expand("~/Dropbox/...")
}

#windows
if(.Platform$OS.type=="windows"){
  root_dir <- path.expand("d://Dropbox/WRF/")
  output_dir <- path.expand("d://Dropbox/WRF/MPEdailyBigBox/")
}

MPE.root <- path.expand("g://DailyNetCDF/")
### proj4 strings
HRAP="+proj=stere +lat_0=39.224079 +lon_0=-98.54180699999999 +k=1 +x_0=0 +y_0=0 +ellps=sphere +units=m +no_defs" #HRAP SR-ORG:4694
LL.WGS84="+proj=longlat +ellps=WGS84"
...
#summary(SB1.WGS84)
ChewedCheese <- readShapePoly(paste(root_dir,"SpatialData/BigBox",sep=""),proj4string=CRS(LL.WGS84))
...
ChewedCheese <- spTransform(ChewedCheese,CRS(HRAP))
summary(ChewedCheese)
#summary(SB1HRAP)
#plot(SB1HRAP)
...
#once the writeOGR is done, can just load projected,buffered shp:
...
GetCoords <- open.ncdf(paste(MPE.root,"ST4_2009010112_24h.nc",sep=""))
...
ConusPts <- spTransform(ConusWGS84,CRS(HRAP))
summary(ConusPts)

plot(ChewedCheese, col="orange")
#plot(ConusPts, add=T)
...
#ID[7354,]
ConusSPDF <- SpatialPointsDataFrame(ConusPts,ID) #this is used later?
...
WSCentroids <- over(ChewedCheese,ConusPts,returnList=T) #'over' doesn't work right with SPDF? just use SP
...
################################
PixelListChewedCheese <- WSCentroids[,1]
Npixels <- length(PixelListChewedCheese) #This is the number of pixels to extract data from, set length of SPDF to this
print(Npixels) 
PixelIDs <- as.vector(PixelListChewedCheese)
#print(PixelIDs)

startdate <- as.Date("2009-10-03")
enddate <- as.Date("2009-10-03")
...
llCounter = 1
for(i in 1:Ndays){
  daily.ncdf.filename[llCounter] <- paste(MPE.root,"ST4_",days.text[i],"12_24h.nc",sep="")
...
MissingDays08 <- array(data=NA,dim=c(Ndays,Npixels))
colnames(MissingDays08) <- PixelIDs
rownames(MissingDays08) <- days.text
write.csv(MissingDays08,file=paste(output_dir,"MPE2008_B.csv",sep=""))

MissingDays09 <- array(data=NA,dim=c(Ndays,Npixels))
colnames(MissingDays09) <- PixelIDs
rownames(MissingDays09) <- days.text
write.csv(MissingDays09,file=paste(output_dir,"MPE2009_B.csv",sep=""))
...
#WSPrecip#set NA value
WSPrecip[,7000:7005]

#Add daily mean precip if you want it:
Means <- rowMeans(Play, na.rm = T, dims = 1)
WSPrecip[,Npixels+1] <- rowMeans(WSPrecip, na.rm=T, dims=1)
colnames(WSPrecip)[Npixels+1] <- "Mean"
WSPrecip[,7000:7005]

#write daily file for year
write.csv(WSPrecip, file=paste(output_dir,"MPE2006_B.csv",sep=""))

######
#Append partial files

A2006 <- read.csv(file=paste(output_dir,"MPE2006.csv",sep=""))
B2006 <- read.csv(file=paste(output_dir,"MPE2006_B.csv",sep=""))
MPE2006 <- rbind(A2006,B2006)
write.csv(MPE2006, file=paste(output_dir,"MPE2006.csv",sep=""))

A2008 <- read.csv(file=paste(output_dir,"MPE2008_A.csv",sep=""))
B2008 <- read.csv(file=paste(output_dir,"MPE2008_B.csv",sep=""))
C2008 <- read.csv(file=paste(output_dir,"MPE2008_C.csv",sep=""))
MPE2008 <- rbind(A2008,B2008,C2008)
write.csv(MPE2008, file=paste(output_dir,"MPE2008.csv",sep=""))

A2009 <- read.csv(file=paste(output_dir,"MPE2009_A.csv",sep=""))
B2009 <- read.csv(file=paste(output_dir,"MPE2009_B.csv",sep=""))
C2009 <- read.csv(file=paste(output_dir,"MPE2009_C.csv",sep=""))
dim(A2009)
dim(B2009)
dim(C2009)
MPE2009 <- rbind(A2009,B2009, C2009)
write.csv(MPE2009, file=paste(output_dir,"MPE2009.csv",sep=""))

MPE2002 <- read.csv(file=paste(output_dir,"MPE2002.csv",sep=""))
MPE2006 <- read.csv(file=paste(output_dir,"MPE2006.csv",sep=""))
MPE2007 <- read.csv(file=paste(output_dir,"MPE2007.csv",sep=""))
MPE2008 <- read.csv(file=paste(output_dir,"MPE2008.csv",sep=""))
MPE2009 <- read.csv(file=paste(output_dir,"MPE2009.csv",sep=""))
...
MPEDailyAll <- rbind(MPE2002,MPE2006,MPE2007,MPE2008,MPE2009)
dim(MPEDailyAll)
write.csv(MPEDailyAll,file=paste(output_dir,"MPEDailyAll.csv",sep=""))
...
#Rain <- as.data.frame(cbind(ExtractID,Rain)) #the extract ID isn't necessary but feels safer
#Rain <- Rain[PixelIDs,]
